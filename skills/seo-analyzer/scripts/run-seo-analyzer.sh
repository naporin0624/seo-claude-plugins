...
# Usage: run-seo-analyzer.sh <file> [--json]
...
ensure_setup() {
    if [ ! -d "$SKILL_DIR/node_modules" ] || [ ! -d "$SKILL_DIR/dist" ] || [ ! -f "$SKILL_DIR/dist/index.js" ]; then
        echo "Setting up seo-analyzer (installing dependencies and building)..." >&2
        ...
        (cd "$ROOT_DIR" && pnpm install --filter seo-analyzer)
...
if [ $# -eq 0 ]; then
    echo "Usage: run-seo-analyzer.sh <file> [options]" >&2
    echo "" >&2
    echo "Options:" >&2
    echo "  --json                 Output in JSON format" >&2
    echo "" >&2
    echo "Examples:" >&2
    echo "  run-seo-analyzer.sh index.html" >&2
    echo "  run-seo-analyzer.sh path/to/page.html --json" >&2
    exit 1
fi
